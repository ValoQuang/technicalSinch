...
describe("EncodeMessage", () => {
    ...
    it("give exceeded header amount error", () => {
        ...
        for (let i = 0; i < EncodeMessage_1.MAX_HEADER_COUNT + 1; i++) {
    ...
    it("give header size error", () => {
        const exceededHeaders = new Map();
        exceededHeaders.set('x'.repeat(EncodeMessage_1.MAX_HEADER_SIZE + 1), 'Some values');
        expect(() => {
            EncodeMessage_1.BinaryMessageEncoder.encodeMessage(exceededHeaders, payload);
    ...
    it("give value size error", () => {
        ...
        exceededValue.set('Header', 'x'.repeat(EncodeMessage_1.MAX_HEADER_SIZE + 1));
        expect(() => {
            EncodeMessage_1.BinaryMessageEncoder.encodeMessage(exceededValue, payload);
        }).toThrowError("Header names and values must be limited to 1023 bytes each.");
    });
    it("give payload size error", () => {
        const exceededPayload = 'x'.repeat(EncodeMessage_1.MAX_PAYLOAD_SIZE + 1);
        expect(() => {
